...
void PPRunConfig::read(const QString& fileName)
{
    ...
    else
    {
        ...
        while (getline(in, ln)) {
            ...
            stringTok(toks, line, ":");
            if (toks.count() == 2) {
                QString groupkey(toks.at(0).simplified());
                QString value(toks.at(1).simplified());
                stringTok(toks, groupkey, "_");
                if (toks.count() == 2) {
//                    QString group(toks.at(0));
//                    QString key(toks.at(1));
                    processParamDef(toks.at(0), toks.at(1), value);
                }
            }
        }
    }
