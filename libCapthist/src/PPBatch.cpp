...
int PPBatch::run(PPGetOpts& opts, PPBatchOutputMaker& out)
{
    ...
    {
        cout << "Capthist: Version " + PPVersion::versionString().toStdString() << endl;
    }
...
int PPBatch::runBatch(PPGetOpts& opts, PPBatchOutputMaker& out)
{
    ...
    out.write(QString("Capthist v.%1").arg(PPVersion::versionString().data()), PPBatchOutputMaker::Emphasis);
    ...
    {
        stringstream ss;
        ss << "Unable to read run configuration file: " << rcFileName.toStdString() << ends;
        out.write(ss.str().data(), PPBatchOutputMaker::Warning);
    }
...
int PPBatch::batch(PPBatchOutputMaker& out)
{
    ...
    for (int run = 0; run < numRuns; run++)
    {
        ...
        else
        {
            ...
            out.write("Processing group \"" + QString(runConfigVector[0].name.data()) + "\"...");
        }
