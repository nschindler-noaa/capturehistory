		...
		if (argv[i][0] == '-')
		{
			...
			else if (key == 'l')
			{
				...
				if ( !value  )
					...
					out.write ("Error. argument to -T should be SURPH1, SURPH2, or ROSTER." );
					settings.setValue( PitProSettings::OutputFormat, "SURPH1" );
				}
			}
			else if (key == 'd')
			{
				if ( strlen( value ) != 8 ) 
					out.write ("Error. argument to -d should be in date format yyyymmdd." );
				else {
			...
			{
				if ( strlen( value ) != 8 ) 
					out.write ("Error. argument to -J should be in date format yyyymmdd." );
				else {
					DateConverter dc( value );
...
string PPFlags::usage()
{
	...
	ss << "capthist: " << PPVersion::version << endl;
	ss << "Usage:" << endl;
