...
public:
    ...
    void setRiverKm( const QString rkm) { riverkm = rkm; }
...
private:
    ...
    bool recap;
    int age;
    QString riverkm; // supplied by rel/mort file, may be empty
};
...
public:
    ...
    bool isMatch (const QString& pitCode) const;
    int compare(const QString &pitCode) const;
    bool isValid () const { return !pitCode.isEmpty (); }
    ...
    void setPitCode (const QString& rhs) { pitCode = rhs; }
    void setReleaseDate (double rd) { releaseDate = rd; }
    void setICovs(const QStringList& icv) { icovs = icv; }
    ...
    void addRecord (const Site* site, cbr::CbrPit::Stage stage, cbr::CbrPit::Outcome outcome,
        double firstDate, double lastDate, int hits);
    void addRecapRecord (const Site* site, double date, const QString& riverkm);
    ...
    // output
    QString hist (const SitesMask& mask) const;
    QString dd (const SitesMask& mask, bool julianDates = true, bool showAll=false) const;
    QString tt (const SitesMask& mask, bool showAll) const;
