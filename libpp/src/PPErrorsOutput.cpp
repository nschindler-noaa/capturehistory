...
QString PPErrorsOutput::getFileName(const QString prefix) const
{
    PitProSettings& settings = PitProSettings::getInstance();
    QString suffix = settings.getValue (PitProSettings::ErrorSuffix);
...
void PPErrorsOutput::writeHeader()
{
    ...
    for ( int i = 0; i < PPErrors::NumErrorTypes; ++i ) {
        ofs << "#\t";
        ofs.width(2);
        ofs << i + 2 << ": " << PPErrors::getText( i ).toStdString() << endl;
    }
    ofs << "#" << endl << endl;
